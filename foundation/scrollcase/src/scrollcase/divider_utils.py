...
def divider_curve(lining_outer_radius, square_height_mm, wall_thickness_mm):
    pts = [
...
def divider_solid(
    lining_outer_radius, square_height_mm, square_loft_radius, wall_thickness_mm
):
    with BuildPart() as divider:
        with BuildSketch():
            with BuildLine() as divider_ln:
                ...
                ln2 = divider_curve(
                    lining_outer_radius, square_height_mm, wall_thickness_mm
                )
                ln3 = Line(
                    (lining_outer_radius - wall_thickness_mm, 0),
                    (square_loft_radius * 2, 0),
