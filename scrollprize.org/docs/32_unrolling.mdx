...
<Head>
  ...
  <script src="https://cdn.jsdelivr.net/npm/katex@0.13.11/dist/contrib/auto-render.min.js" async defer onload="renderMathInElement(document.body);"></script>
</Head>
...
It's a mess! The papyrus layers, that survived a volcanic eruption and were buried under lava and mud for centuries, are carbonized, damaged, frayed and, most importantly, still wrapped!

To read a scroll, we first need to unroll it... **virtually**!

If in the chaotic 3D image you are looking at you could isolate a contiguous part of the papyrus sheet, you could then fit a 2-manifold (or an orientable genus-0 developable surface) to it and find a bidimensional isometric parametrization [^1] that defines a base on top of it.

The 2-manifold is commonly represented as a non-watertight triangular mesh. When rendered in the coordinate frame given by its base, the surface looks automatically flat if the found parametrization is obtained with an isometric map from 3D to 2D, i.e. locally preserving distances, shapes, and angles.

<div className="mb-4">
  <img src="/img/virtual-unrolling/s5-segment.png" className="w-[60%]"/>
  <figcaption className="mt-[-6px]">A triangular mesh in 3D space.</figcaption>
</div>

A triangular mesh can represent part of the papyrus surface in 3D.
...
The same mesh rendered in 2D. Faint letters appear.  

When the 3D mesh is rendered in 2D, faint letters begin to appear. Machine learning can enhance the visibility of the text.


The open problem that has been puzzling our community for more than one year, and Dr. Seales’ research for over twenty-five years, has an extremely easy statement:

> **Find an automated way to isolate the 2-manifold that represents the rolled scroll in the 3D images.**
...
<div className="mb-4">
  <img src="/img/virtual-unrolling/ir.png" className="w-[70%]"/>
...
All approaches rely also on the information about the scroll umbilicus.
...
**(Overview)**  
This method leverages the idea that the scroll, in its original form, was a 2D spiral with approximate cylindrical symmetry. Once flattened, the papyrus sheet is nearly rectangular, with locally perpendicular fibers. The approach attempts to find the transformation that deformed this canonical spiral into the observed data.

<div className="mb-4">
  <img src="/img/virtual-unrolling/spiral.jpg" className="w-[70%]"/>
...
<div className="mb-4">
  <img src="/img/virtual-unrolling/fibers.png" className="w-[70%]"/>
...
<div className="mb-4">
  <a href="/img/virtual-unrolling/fasp-spiral-fitting.jpg" target="_blank" rel="noopener noreferrer">
    <img src="/img/virtual-unrolling/fasp-spiral-fitting.jpg" className="w-[70%] cursor-pointer" />
  </a>
  <figcaption className="mt-[-6px]">FASP - Spiral Fitting submission</figcaption>
</div>
...
<div className="mb-4">
  <img src="/img/virtual-unrolling/surface-predictions.jpg" className="w-[70%]"/>
...
- $B(\mathbf{x}_{new}, M_{\text{current}})$ measures how adding the new point affects the mesh’s local curvature (flatness).
...
<div className="mb-4">
  <a href="/img/virtual-unrolling/fasp-surface-tracer.jpg" target="_blank" rel="noopener noreferrer">
    <img src="/img/virtual-unrolling/fasp-surface-tracer.jpg" className="w-[70%] cursor-pointer" />
  </a>
  <figcaption className="mt-[-6px]">FASP - Surface tracer submission</figcaption>
</div>
...
A limitation is that compressed regions may yield lower point-cloud density, cross-fiber edges can be falsely extracted, along as scan artifacts (beam hardening) can be mistakenly detected as false surfaces.

<div className="mb-4">
  <img src="/img/virtual-unrolling/thaumato-ir.png" className="w-[70%]"/>
  <figcaption className="mt-[-6px]">Slice showing surface point clouds (blue) in Scroll 5 (PHerc 172).</figcaption>
</div>

#### Geometry Processing
The resulting point cloud is divided into overlapping chunks. Within each chunk, machine learning instance segmentation (i.e., Mask3D [^4]) clusters the points into patches belonging to different wraps of the scroll. The cluster instances are then mapped to a graph where possibly adjacent instances are linked by edges. The instances are then stitched together by solving a global connectivity graph problem that assigns consistent “winding angles” around the scroll umbilicus to each patch, forming a single coherent mesh, and correcting segmentation errors.

Mathematically, Thaumato Anakalyptor seeks to find a winding angle assignment function $ f: N \rightarrow \mathbb{R} $, where $N$ are the nodes in the graph and $f(node)$ is a winding angle.
...
<div className="mb-4">
  <a href="/img/virtual-unrolling/thaumato-s5jan.jpg" target="_blank" rel="noopener noreferrer">
    <img src="/img/virtual-unrolling/thaumato-s5jan.jpg" className="w-[70%] cursor-pointer" />
  </a>
  <figcaption className="mt-[-6px]">Virtual unrolling attempt with Thaumato Anakalyptor on Scroll 5.</figcaption>
</div>
...
Despite these advances, the problem is still unsolved. We need your help to virtual unroll the Herculaneum scrolls.
