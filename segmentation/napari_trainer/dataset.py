...
class NapariDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for t_name, volumes_list in self.target_volumes.items():
            ...
            is_2d = len(label_arr.shape) == 2
            
            if is_2d:
                # use only y, x dimensions
                ...
                if out_c == 1:
                    label_patch = label_patch[None, ...]  # => (1, H, W)
            else:
            ...
            if "image" not in data_dict:
                ...
                if is_2d_img:
                    # For 2D data, apply 2D transformations directly to 2D data
                    if self.image_transforms:
                        transforms = self.image_transforms
                        transformed = transforms(image=img_patch)
