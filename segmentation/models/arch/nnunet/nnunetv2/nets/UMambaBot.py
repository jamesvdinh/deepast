...
def get_umamba_bot_from_plans(plans_manager: PlansManager,
    ...
    """
    Modified to handle both old and new configuration formats
    """
    # Try to get kernel sizes - this will handle both formats now
    ...
    # Get values using the configuration manager properties that handle both formats
    ...
    n_conv_per_stage_encoder = configuration_manager.n_conv_per_stage_encoder
    ...
    model = network_class(
        ...
        features_per_stage=[min(base_num_features * 2 ** i,
                              max_num_features) for i in range(num_stages)],
        conv_op=conv_op,
